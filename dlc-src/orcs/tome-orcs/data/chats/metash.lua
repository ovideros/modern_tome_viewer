...
newChat{ id="w-welcome",
	text = _t[[Hail, @playername@!]],
	answers = {
		{_t"Yes?", jump="w-explain", cond=function(npc, player) return not player:hasQuest("orcs+krimbul") end},
		{_t"Our is free Metash, the tyrant is no more.", jump="w-thanks", cond=function(npc, player) return player:hasQuest("orcs+krimbul") and player:isQuestStatus("orcs+krimbul", engine.Quest.DONE) end},
		{_t"Not now."},
	}
}

newChat{ id="w-explain",
	text = _t[[I came here to warn the Kruk Pride of the threat Nektosh poses and ask for their help, but they have some more immediate threats to deal with...  We should help them repel these Steam Giants.  They are the only people who have ever treated us with respect and dignity; if they are crushed by the Atmos Tribe or the Allied Kingdoms, we will surely be next.  Their success is our survival.
