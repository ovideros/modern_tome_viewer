...
local basesize = math.sqrt((engine.Map.tile_w*engine.Map.tile_w)+(engine.Map.tile_h*engine.Map.tile_h))
...
local function make_beam(fork_i)
	...
	local size = 2
	local basedir = rng.float(0, math.pi*2)
	local starta = basedir+math.pi/2
	local starts = rng.range(10, 20)
...
return { engine=core.particles.ENGINE_LINES, blend_mode=core.particles.BLEND_SHINY, generator = function(id)
	...
	local ret = {
		...
		r = 0xE7/255, rv = 0.05, ra = 0,
		g = 0x1E/255, gv = 0.2 / 6, ga = 0,
		b = 0x05/255, bv = 0.3 / 6, ba = 0,
		a = 0.2, av = 0, aa = 0.001,
	}
	last_id = id
	return ret
