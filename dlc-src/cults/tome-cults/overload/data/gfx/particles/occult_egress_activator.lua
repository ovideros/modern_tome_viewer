...
return { blend_mode = core.particles.BLEND_SHINY, generator = function()
	...
	local life = rng.range(5, 15)

	return {
		life = life, trail=1,
		size = rng.range(2, 5), sizev = -0.2, sizea = 0,

		x = bx + r * math.cos(angle) + math.cos(rightAngle) * offset, xv = 0, xa = 0,
		y = by + r * math.sin(angle) + math.sin(rightAngle) * offset, yv = 0, ya = 0,
		dir = angle + math.rad(180), dirv = 0, dira = 0,
		vel = rng.range(1, 3), velv = 0, vela = 0,

		r = 0, rv = 0, ra = 0,
		g = rng.float(0.4, 0.6), gv = 0, ga = 0,
		b = rng.float(0.8, 1.0), bv = 0, ba = 0,
		a = 1, av = -1 / life, aa = 0,
	}
end, },
function(self)
