...
local dir = dir
local dist = (dist or 1) * engine.Map.tile_w
...
for i = 1, 16 do
	local x = 0
	local x = slashdir * i^1.9 * 0.8 / sizeratio
	local y = -dist / 16 * i
	local a = math.atan2(x, y) + math.pi
	local si = math.sqrt(i) * 0.8
	local size
	if i <= 4 then size = 4 + i * 3
	else size = points[#points].size * 0.92
	end
	points[#points+1] = {angle=0, vel=points[#points].vel+slashdir*rng.float(-si,-si/2)/sizeratio, size=(i==16) and 1 or size, x=x, y=y, prev=#points-1}
end
local nbp = #points
...
nbp, "particles_images/tentacle_segment"
