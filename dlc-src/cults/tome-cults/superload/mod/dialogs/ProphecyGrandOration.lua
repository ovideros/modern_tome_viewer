...
local function spellbound(who, t)
	for tid, active in pairs(who.sustain_talents) do
		if active then
			...
			if tt.type[1]:find("^demented/doom") and t.mode ~= "passive" then
...
function _M:init(actor)
	...
	Dialog.init(self, _t"Grand Oration", game.w * 0.6, game.h * 0.8)
...
function _M:generateList()
	...
	for j, t in pairs(self.actor.talents_def) do
		if self.actor:knowTalent(t.id) and t.type[1]:find("^demented/prophecy") and not spellbound(self.actor, t.id) then
			local nodes = talents
	...
	list = {
		{ char='', name=(_t'#{bold}#Choose a talent#{normal}#'):toTString(), status='', hotkey='', desc=_t"All prophecies that can be used with Grand Oration.", color=function() return colors.simple(colors.LIGHT_GREEN) end, nodes=talents, shown=true },
		chars = chars,
	}
	self.list = list
end
