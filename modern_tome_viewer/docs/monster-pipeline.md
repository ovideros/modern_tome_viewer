...
源码确实为 Actor 提供了独立的速度设置。现在管线会沿着 NPC 模板的继承链读取
这些**字面量字段**，并把有明确设置或继承设置的值写入
`public/data/monsters.json`；怪物详情的「速度」区块会按百分比显示。没有列出的
字段不是 0，而是沿用引擎的 1.0（100%）默认值。本版本 812 个可遇模板中，有
175 个带有至少一项明确的速度覆盖。

| 页面名称 | 源码字段 | 100% 的含义 | 作用 |
| --- | --- | --- | --- |
| 全局速度 | `global_speed_base` | 标准行动节奏 | 整体影响能量积累与行动时间流逝 |
| 移动速度 | `movement_speed` | 标准移动速度 | 只影响移动行动 |
| 战斗速度 | `combat_physspeed` | 标准物理行动速度 | 影响物理攻击与武器行动 |
| 法术速度 | `combat_spellspeed` | 标准施法速度 | 影响法术行动 |
| 精神速度 | `combat_mindspeed` | 标准精神行动速度 | 影响精神能力行动 |

这些字段是倍率而不是百分点评分：200% 表示该类行动的速度为基准的两倍，通常
意味着相同时间内能更快积累行动能量；具体一次行动的实际间隔，还会结合行动本身
的能量消耗、武器或攻击定义中的 `physspeed`，以及技能、装备、状态和难度修正。
因此图鉴展示的是**源码模板基础倍率**，不是对战斗回合中实际出手次数的完整模拟。

游戏引擎中的关键关系是：`Actor:recomputeGlobalSpeed()` 根据
`global_speed_base` 与动态的 `global_speed_add` 得到最终全局速度；战斗接口则用
`combat_physspeed`、`combat_spellspeed`、`combat_mindspeed` 的倒数计算相应行动
耗时，移动接口同样按 `movement_speed` 的倒数计算移动耗时。临时效果、装备、天赋、
随机首领修正等都可能在模板值之外改变最终结果。

当前 1.7.6 源码中的几个核对样例：

| 模板 | 当前源码中的字段 | 图鉴显示 |
| --- | --- | --- |
| 蜜蜂群（`bee swarm`） | 继承 `BASE_NPC_INSECT.global_speed_base = 2` | 全局速度 200% |
| 刀锋恐魔（`blade horror`） | `combat_physspeed = 4` | 战斗速度 400% |
| 链锯恐魔（`saw horror`） | `combat_physspeed = 5` | 战斗速度 500% |
| 白色巨蚁（`giant white ant`） | `global_speed_base = 1.1` | 全局速度 110% |
| 啮木巨蚁（`giant carpenter ant`） | `movement_speed = 1.3` | 移动速度 130% |

这也解释了引述中“高速怪物一回合出手很多次”的现象方向：全局速度更高时，怪物
能更快获得行动机会；若其物理战斗速度或攻击自身速度也更快，行动间隔还会进一步
缩短。不过引述中的“蚂蚁 150%”“蜜蜂 200%”“刀锋恐魔 200%”“链锯恐魔 400%”
并不完全对应当前 1.7.6 源码：当前源码核对到的是蜜蜂 200%、刀锋 400%、链锯
500%，而巨蚁的相关值是白蚁全局 110%、啮木巨蚁移动 130%。这通常意味着资料来自
其他版本、其他怪物变体，或混用了全局／移动／战斗速度的口径；页面按实际扫描到的
1.7.6 模板值展示，不强行套用外部数字。

本节涉及的源代码位置包括：`mod/class/Actor.lua` 的速度初始化与全局速度重算、
`mod/class/interface/Combat.lua` 的移动／战斗／法术／精神速度计算，以及
`data/general/npcs/swarm.lua`、`horror.lua` 和相关 DLC NPC 文件中的模板字段。
...
```bash
npm run test:monsters   # 含速度字段核对：继承、繁殖参数、随机组、等级规则、描述与汉化、种类中文与筛选、图片、产物不变量、前端检索
npm run smoke           # 116 项：含怪物页渲染、搜索、大类/亚类筛选、详情面板
npm run e2e             # 312 项：真实浏览器，含怪物速度详情、大类/亚类筛选与深链、技能面板三档宽度、手机端下拉选择、底部抽屉自身可滚动
```
