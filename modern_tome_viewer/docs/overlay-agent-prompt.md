...
自动提取已经拿到 3650 条（97.3%），加上已手写的 1053 条覆盖层；剩下的因为写法太花
（局部变量、别的技能、条件倍率、自定义 helper…）没拿到。你要处理的就是这些。
...
3. **整条阶梯的整数点只用一种显示约定读得出来**——一次 `tformat` 调用用同一个格式符打印整条阶梯，
   不能"这点截断、那点四舍五入"。混着读也能凑上，说明这条数值与源码/导出不自洽，请记成疑点而不是提交。
