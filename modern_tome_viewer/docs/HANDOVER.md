...
| `scripts/scaling.test.mjs` | 40 项测试（含"全量源码匹配项在每个参考点都吻合"、轴推导、角色等级阶梯、本轮新增的构造器/索引/幂/条件/多强度/他技能等级） |
...
| 轴无法确定 | 34 | 标题里同时有两个以上变化的输入（如 意志 + 法术强度 双阶梯） |
...
- **强度滑条按类型分开：法术强度 / 精神强度 / physical power / steampower，1–200，是"有效值"**（`POWER_RANGE`）：
  ...
  - 有效 100 需要原始 300，有效 200 需要原始 1100，所以 1–200 覆盖全部真实角色；
