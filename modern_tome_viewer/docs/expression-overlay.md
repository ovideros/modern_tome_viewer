...
标题里声明过——即 `表达式消耗 ⊆ 标题声明`（轴除外），**且**每条阶梯的整数点只用**一种**显示约定读得出来
（见文末「一条阶梯不能混用显示约定」）。任一不满足即为 FAIL。
...
标题常常带**不止一条**阶梯（导出会把整个 `info` 函数碰到的参数并集抄进每一个 acronym）。
如果这些阶梯**逐元素完全相同**（例如 `physical save` / `spell save` / `mental save` 都是
`10,25,50,75,100`），引擎是把它们**一列一列同时推进**的，它们算**同一条轴**：

- 你可以直接读其中任意一条，例如 `["actor","spell save"]`，判分器会让三条一起随轴变化；
- 输入集合检查里，轴本身被排除，其余同阶梯标签算「已声明」——所以
  `标题声明 [mental save, spell save] vs 表达式消耗 [spell save] ✅ 覆盖（标题是超集）` 是正常的；
- 但**不要挑一条来代表全部**就算完事：如果真公式是 `max(敏捷, 魔力)`，请照实写成 `max`，
  否则两条滑条里有一条会变成摆设。只有当节点不支持嵌套（例如 `statScale` 的第一个参数
  只能是属性名）时，才退化为单条并在 `note` 写明。

**陷阱**：几条阶梯数值相同时，15 点判分**分辨不出**哪一行读的是哪一个输入
（无尽追踪三行都读豁免，但读哪个豁免只能由句子决定）。这种情况下必须在 `note` 里写清映射依据。

**仍然无解**：两条阶梯**真的不同**时（例如 `魔力 10..100` 配 `角色等级 1..50`），
那是真正的联合轴，本工作台不支持，工具会报「标题里没有唯一可变的轴」。

## 说明：一条阶梯不能混用显示约定

一次 `tformat` 调用用**同一个格式符**打印整条阶梯的五个数，所以不能"这点截断、那点四舍五入"。
判分器会要求整条阶梯的整数点能被**同一个**约定读完（`trunc` / `round`，
以及导出会做的第二次取整 `%0.1f`、`%0.2f`）。如果你发现只有混着读才能凑上，
**那不是你找到了窍门，是这条数值与源码/导出不自洽**——请记成疑点，不要提交。
